...
public class UserXLeagueXPlayerService {
    ...
    public void linkUserLeaguePlayer(UUID userID, UUID leagueID, UUID playerID) {
        ...
        userXLeagueXPlayerRepository.save(new UserXLeagueXPlayer(new UserXLeagueXPlayerId(playerID, leagueID, userID,1), 0));
    }
    ...
    public Set<PlayerTeamInfoPOJO> getUserXLeagueTeam(UUID userID, UUID leagueID) {
        ...
        teamres.forEach(tentry -> {
            ...
            PlayerTeamInfoPOJO ptInfoPOJo = new PlayerTeamInfoPOJO(player.getUuid(), player.getUsername(), player.getFullname(), player.getRole(), tentry.getAligned());
            team.add(ptInfoPOJo);
        });
        return team;
    }
